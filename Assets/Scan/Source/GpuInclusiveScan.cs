...
public class GpuInclusiveScan: IScan
{
   private const string KERNEL_NAME_SCAN       = "CSInclusiveScan";
   ...
   public GpuInclusiveScan(ComputeShader s)
   {
      shader = s;
   ...
   private void HandleSumBuffer(ComputeBuffer dataBuffer, int length)
   {
      ...
      var sumBuffer = new ComputeBuffer(size, sizeof(int));
